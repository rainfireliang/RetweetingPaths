---
title: "Reconstruct Diffusion Cascades"
output:
...
To reconstruct a diffusion cascade requires two datasets: 1, all retweets from an origanl tweet, which can be obtained from Twitter API - "GET statuses/retweets/:id""; 2, the following relationships among the retweeters, which can be ontained from the "GET friends/ids" API.

The retweet dataset should be a dataframe with the three columns: sharers, seed_users, and time. Each row indicates a retweeting action.Retweets were ordered chronologically from earliest to most recent. The below example is from Figure 2A.
...
The following function can be used to recontruct a diffusion cascade, given retweets and followings.
...
```{r}
diffnet = recontruct(retws=retws,followings=followings)
diffnet
```
